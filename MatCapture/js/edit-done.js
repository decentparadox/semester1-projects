...
$(document).ready(async function () {
    ...
    if (window.nimbus_core.is_chrome) {
        $('#nsc_button_youtube').on('click', async function () {
            $('#nsc_button_youtube .nsc-icon').addClass('animate-rotate')

            const premium = nscNimbus.checkPremium(true, 'premium_banner_youtube')

            if (!premium) {
                $('#nsc_button_youtube .nsc-icon').removeClass('animate-rotate')
                return
            }

            if (await youtubeShare.getToken()) {
                $('#nsc_button_youtube .nsc-icon').removeClass('animate-rotate')
...
// localStorage.appType = 'google';
    ...
    $('#nsc_button_google').on('click', async function () {
        $('#nsc_button_google .nsc-icon').addClass('animate-rotate')

        const premium = await nscNimbus.checkPremium(true, 'premium_banner_google_drive')

        if (!premium) {
            $('#nsc_button_google .nsc-icon').removeClass('animate-rotate')
            return
        }

        if (await googleShare.getToken()) {
            $('#nsc_button_google .nsc-icon').removeClass('animate-rotate')
    ...
    $('#nsc_screen_name').on('input', function () {
        this.value = this.value.replace(/[^A-Za-z\d-_\.\s]/g,'');
    })
    ...
    $('#nsc_environment_info').on('change', function () {
        ...
        let info = '\n\n-----------------\n' +
            (nscExt.getOption('lastTypeScreenshot') === 'capture-window' ? '' : chrome.i18n.getMessage('nimbusInfoPage') + ': ' + pageInfo.url + '\n') +
        ...
        .on('click', async function () {
            ...
            if ($(this).data('type') === 'youtube') {
                if (nimbus_screen.info.file.format === 'gif') {
                    ...
                    $('#nsc_button_youtube').trigger('click')
                }
...
chrome.runtime.onMessage.addListener(function (req) {
    if (req.operation === 'page_extension_activated') {
        console.log('page_extension_activated')
