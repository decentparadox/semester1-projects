...
var youtubeShare = {
    isGetTokenProgress: false,
    type: null,
    channel: null,
    playlist: [],
    scopes: ['https://www.googleapis.com/auth/youtube.upload https://www.googleapis.com/auth/youtubepartner'],
    getToken: async () => {
        const token = await nscCore.getAuthToken({scopes: this.scopes})
        console.log('getToken', token)
        localStorage.setItem('access_token_youtube', token);
        return token;
    },
    auth: async () => {
        const token = await nscCore.getAuthToken({scopes: this.scopes, interactive: true})
    ...
    //     if(youtubeShare.isGetTokenProgress) {
    //         return;
    //     }
    //     youtubeShare.isGetTokenProgress = true;
    //
    ...
    clearData: async () => {
        if (await youtubeShare.getToken()) {
            chrome.identity.removeCachedAuthToken({token: localStorage.access_token_youtube})
            // chrome.runtime.sendMessage({operation: 'oauth_youtube_logout'});
    ...
    httpRequest: function (method, url, data, headers, cb, popup) {
        ...
        youtubeShare.xhr.setRequestHeader('Authorization', 'Bearer ' + localStorage.access_token_youtube);
