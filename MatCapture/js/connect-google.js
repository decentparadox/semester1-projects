...
window.googleShare = {
    ...
    logout: async function () {
        if (await googleShare.getToken()) {
            chrome.identity.removeCachedAuthToken({token: localStorage.accessTokenGoogle}, () => {
                console.log('logout')
            })
    ...
    server: {
        xhr: null,
        abort: function () {
            if (googleShare.server.xhr) {
                googleShare.server.xhr.abort();
            }
        },
    ...
    //     if(googleShare.isGetTokenProgress) {
    //         return;
    //     }
    //     googleShare.isGetTokenProgress = true;
