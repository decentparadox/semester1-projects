(async function () {
    ...
    let nsc_content_camera = {
        ...
        showCamera: async function (save) {
            ...
            nsc_content_camera.iframe = true;
        ...
        getElemAppend: () => {
            return !!document.fullscreenElement ? document.fullscreenElement : document.documentElement
        },
        html: async () => {
            if ($('.nsc-content-camera').length) {
                return;
            }
            const html = await fetch(chrome.runtime.getURL('template/panel-video-camera.html')).then(res => res.text());
            $(nsc_content_camera.getElemAppend()).append(html);
            ...
            document.addEventListener("fullscreenchange", async () => {
                $(nsc_content_camera.getElemAppend()).append($('.nsc-content-camera'));
            })
