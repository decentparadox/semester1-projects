...
//                canvas.cache.data = canvas.fon.context.getImageData(0, 0, canvas.common.width, canvas.common.height);
        ...
        this.loadBackgroundImage = function (img, cb) {
            if (typeof img === 'object') {
                ...
                const width = Math.ceil(w * z)
                const height = Math.ceil(h * z)

                $(_this).width(width).height(height);
        ...
        this.setWaterMark = function (dataURL, data, cb) {
            for (let i = objects.length - 1; i >= 0; --i) {
                if (objects[i].type === 'img' && objects[i].watermark) {
                    objects.splice(i, 1);
                }
            }

            if (arguments.length >= 2) {
                this.loadImageObject(dataURL, Object.assign(data, {watermark: true}), cb)
            } else {
                drawAll();
            }
        };
