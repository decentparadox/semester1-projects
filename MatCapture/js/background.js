...
if (localStorage.appVersion !== version) {
    localStorage.removeItem('popupInfoDesktopVersionFirstClick')
}
...
(async function () {
    ...
    if (storage.firstInstall) {
        ...
        if (!window.nimbus_core.is_app && storage.appType === 'nimbus') {
            ...
            await nscCore.tabCreate({
                url: 'https://nimbusweb.me/capture-new-wizard/?utm_source=capture&utm_medium=addon&utm_campaign=welcom_page',
                active: true
            })
        ...
        entire: {
            ...
            captureNew: async (data) => {
                ...
                if (data.status === 'finish' || data.status === 'cancel') {
                    ...
                    console.log(localStorage.imageFileInfo)
...
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
        ...
        switch (request.operation) {
            ...
            case 'oauth_google':
                google_oauth.login()
                break
            case 'oauth_google_logout':
                google_oauth.logout()
                break
            case 'oauth_youtube':
                window.youtube_oauth.login()
                break
            case 'oauth_youtube_logout':
                window.youtube_oauth.logout()
                break
            case 'update_menu':
                screenshot.createMenu()
                break
...
chrome.windows.onFocusChanged.addListener(
    async (windowId) => {
        const [tab] = await nscCore.tabQuery({active: true, windowId});
...
chrome.tabs.onHighlighted.addListener(async function (info) {
...
window.google_oauth = {
    ...
    is_tab: false,
    login: async () => {
        chrome.identity.getAuthToken({scopes: this.scopes}, (token) => {
            console.log('token1', token)
            if(!token) {
                chrome.identity.getAuthToken({interactive: true, scopes: this.scopes}, (token) => {
                    console.log('token2', token)
                    localStorage.accessTokenGoogle = token
    ...
    logout: async () => {
        console.log({token: localStorage.accessTokenGoogle})
        await chrome.identity.removeCachedAuthToken({token: localStorage.accessTokenGoogle})
    }
}
    ...
    login: function () {
        ...
        chrome.identity.getAuthToken({scopes: this.scopes}, (token) => {
            if(!token) {
                chrome.identity.getAuthToken({interactive: true, scopes: this.scopes}, (token) => {
                    console.log(token);
                    localStorage.access_token_youtube = token
