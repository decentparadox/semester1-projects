...
$(document).ready(async function () {
    ...
    $('input, button, select, textarea').on('input change auto-input', async function (e) {
        ...
        switch (this.name) {
            ...
            case 'type-watermark':
                ...
                if (localStorage.watermarkType === 'text' || localStorage.watermarkType === 'timestamp') {
    ...
    $('button[name=filename-template-text-watermark]').on('click', function () {
        let $name = $('#filename_template_text_watermark')
        $name.val($name.val() + '{' + this.value + '}').trigger('input')
    })
