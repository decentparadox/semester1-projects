...
function showRecordSetting () {
    ...
    //     $('#nsc_flags_size_video').addClass('nsc-hide')
    // } else {
    //     $('#nsc_flags_size_video').removeClass('nsc-hide')
    // }
}
...
$(async function () {
    ...
    $('.nsc-popup-info-box button').on('click', function () {
        $(this).closest('.nsc-popup-info-box').addClass('nsc-hide')
    })

    const nimbusAuthorized = nscExt.getOption('nimbusAuthorized');
    const popupInfoDesktopVersionFirstClick = nscExt.getOption('popupInfoDesktopVersionFirstClick') || 0;
    const popupInfoNimbusNoteFirstClick = nscExt.getOption('popupInfoNimbusNoteFirstClick') || 0;
    const popupInfoCaptureProFirstClick = nscExt.getOption('popupInfoCaptureProFirstClick') || 0;
    const popupInfoCaptureProCountShow = nscExt.getOption('popupInfoCaptureProCountShow') || 0;
    const popupInfoCaptureProTimeFirstShow = nscExt.getOption('popupInfoCaptureProTimeFirstShow') || 0
    // const popupCaptureActionsBottomLinkCountShow = nscExt.getOption('popupCaptureActionsBottomLinkCountShow') || 0

    $('#nsc_popup_info_desktop_version .nsc-button').on('click', () => {
        nscExt.setOption('popupInfoDesktopVersionFirstClick', Date.now())
    })

    $('#nsc_popup_info_capture_pro .nsc-button').on('click', () => {
        nscExt.setOption('popupInfoCaptureProFirstClick', Date.now())
    })

    $('#nsc_popup_info_nimbus_note .nsc-button').on('click', () => {
        nscExt.setOption('popupInfoNimbusNoteFirstClick', Date.now())
    })

    if(!popupInfoDesktopVersionFirstClick) {
        // if(is_macintosh || is_linux) {
            nscExt.setOption('popupInfoDesktopVersionFirstClick', Date.now())
        // } else {
        //     $('#nsc_popup_info_desktop_version').removeClass('nsc-hide')
        // }
    }

    if(popupInfoDesktopVersionFirstClick && popupInfoDesktopVersionFirstClick <= Date.now() - 48 * 3600 * 1000) { // прошло более 48 часов с предыдущего окна
        if(!popupInfoCaptureProFirstClick // пользователь не нажал на кнопки
        && !nscExt.getOption('nimbusPremium') // нету про
        && (popupInfoCaptureProCountShow <= 5 // показали не менее 5 раз
        || (popupInfoCaptureProTimeFirstShow && popupInfoCaptureProTimeFirstShow >= Date.now() - 5 * 24 * 3600 * 1000)) // показали не менее 5 дней назад
        ) {
            $('#nsc_popup_info_capture_pro').removeClass('nsc-hide')
            $('#nsc_button_open_nimbus_premium').addClass('nsc-hide')
            nscExt.setOption('popupInfoCaptureProCountShow', popupInfoCaptureProCountShow + 1)

            $(document).on('user_premium_change', function () {
                $('#nsc_button_open_nimbus_premium').addClass('nsc-hide')
            })

            if(!popupInfoCaptureProTimeFirstShow) {
                nscExt.setOption('popupInfoCaptureProTimeFirstShow', Date.now());
            }
        } else if(!popupInfoNimbusNoteFirstClick) {
            $('#nsc_popup_info_nimbus_note').removeClass('nsc-hide')
        }
    }

    if(nimbusAuthorized) {
        $('#nsc_capture_actions_bottom_link').removeClass('nsc-hide')

        if(!is_windows) {
            $('#nsc_capture_actions_bottom_link_desktop').addClass('nsc-hide')
        }
        // if(popupCaptureActionsBottomLinkCountShow >= 5 && popupCaptureActionsBottomLinkCountShow < 10) {
        //     $('#nsc_capture_actions_bottom_link').removeClass('nsc-hide')
        // }
        // nscExt.setOption('popupCaptureActionsBottomLinkCountShow', popupCaptureActionsBottomLinkCountShow + 1)
    }
    ...
    if (quickCapture !== 'false' && !recordStatus && !entireStatus) {
        ...
        if (quickVideoCapture !== 'false') {
            ...
            if (streamLoading) {
                ...
                const auth = nscExt.getOption('nimbusAuthorized');

                if(auth) {
                    $('.nsc-capture-title-pro').find('span').text(premium ? 'pro' : 'basic')
                }
